DROP TABLE IF EXISTS mart.country_candidates;

CREATE TABLE mart.country_candidates AS
WITH delivered_orders AS (
    ...
    WHERE o.status = 'delivered'
     ...
     country_stats AS (
         ...
         GROUP BY o.country_iso, co.name, co.lat, co.lon
     ),
     ...
     distances AS (
         SELECT
             ...
             AVG(
                     6371 * 2 * ASIN(
                             SQRT(
                                     POWER(SIN(RADIANS((o.order_lat - cs.country_lat) / 2)), 2)
                                         + COS(RADIANS(cs.country_lat))
                                         * COS(RADIANS(o.order_lat))
                                         * POWER(SIN(RADIANS((o.order_lon - cs.country_lon) / 2)), 2)
                             )
                                )
             ) AS avg_distance_km
         FROM country_stats cs
                  CROSS JOIN delivered_orders o
         GROUP BY cs.country_iso
     ),
     ...
     combined AS (
         ...
         FROM country_stats cs
                  JOIN distances d ON d.country_iso = cs.country_iso
                  CROSS JOIN totals t
     ),
